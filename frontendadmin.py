...
def insert_text():
    backend.insert(lic_text.get(),item_text.get(),weight_text.get(),price_text.get(),qua_text.get(),op_text.get(),mfr_text.get())
    list1.delete(0,END)
    list1.insert(END,lic_text.get(),item_text.get(),weight_text.get(),price_text.get(),qua_text.get(),op_text.get(),mfr_text.get())

def update_text():
    backend.update(lic_text.get(),item_text.get(),weight_text.get(),price_text.get(),qua_text.get(),op_text.get(),mfr_text.get())

def view_text():
    list1.delete(0,END)
    for rows_view in backend.view():
        list1.insert(END,rows_view)

def search_text():
    list1.delete(0,END)
    for rows_search in backend.search(lic_text.get(),item_text.get()):
        list1.insert(END,rows_search)

def delete_text():
    backend.delete(lic_text.get())
