...
import backend_user
def insert_text():
    backend_user.insert(item_text.get(),weight_text.get(),price_text.get(),qua_text.get())
    list1.delete(0,END)
    list1.insert(END,item_text.get(),weight_text.get(),price_text.get(),qua_text.get())

def view_text():
    list1.delete(0,END)
    for rows_view in backend_user.view():
        list1.insert(END,rows_view)

def search_text():
    list1.delete(0,END)
    for rows_search in backend_user.search(item_text.get()):
...
b1=Button(window,text="Submit",command=insert_text,bg="cyan2",fg="black",font="Times 18 bold")
b1.grid(row=5,column=0)

b2=Button(window,text="Search",command=search_text,bg="cyan2",fg="black",font="Times 18 bold")
b2.grid(row=5,column=2)

b3=Button(window,text="View",command=view_text,bg="cyan2",fg="gray1",font="Times 18 bold")
